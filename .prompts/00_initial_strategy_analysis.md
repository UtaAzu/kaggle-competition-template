...
1. **docs/フォルダ内の全ファイルを読み込む**
   - `docs/01_overview.md` - コンペの概要
   - `docs/02_data_description.md` - データの詳細説明
   - その他すべてのmarkdownファイル

2. **コンペタイプを自動判定する**
   - データ形式から判断：テーブルデータ 📊 / 画像データ 🖼️ / テキストデータ 📝 / マルチモーダル 🎭
   - タスクタイプを判断：分類 / 回帰 / セグメンテーション / 物体検出 / 生成 など

3. **評価指標を確認する**
   - 評価指標の種類（AUC, F1, RMSE, IoU など）
   - 評価指標の実装が必要か（特殊な指標の場合）

**この情報を踏まえて、以下の分析を進めてください。**

---

## 【期待する出力形式】

あなたの分析結果は、以下の構造で出力してください：

```markdown
# 【コンペタイプ】: [テーブル/画像/テキスト/マルチモーダル]
# 【タスク】: [分類/回帰/セグメンテーション等]
# 【評価指標】: [AUC/F1/RMSE等]

## Phase 1: データの違和感分析（0-2h）
[具体的な分析結果]

## Phase 2: 特徴量/前処理戦略（2-4h）
[具体的な実装方針]

## Phase 3: モデル選定と検証（4-6h）
[具体的なモデルとCV戦略]
...
#### 【Phase 1の成果物】
- **このコンペにおける**データの「違和感」を言語化した仮説（2〜3文）
- **このコンペにおける**追加データ戦略の結論（Concat/Stats/使わない）
- **このコンペにおける**CV戦略の方針（StratifiedKFold/GroupKFold/TimeSeriesSplit等）

**⚠️ 重要:** 上記の判断は、必ず`docs/`フォルダ内の公式ドキュメントを読んだ上で行ってください。一般論ではなく、**このコンペ固有の分析**を提供してください。
...
#### 【Phase 2の成果物】
- **このコンペで効く必殺技**の発見と実装方針（1〜2個）
  - 例：「このコンペはSyntheticデータなので、Count Encodingが効きます」
  - 例：「このコンペは医療画像なので、強いAugmentationが効きます」
- シンプルで再現性の高い特徴量生成パイプラインの設計

**⚠️ 重要:** 一般論ではなく、**このコンペのデータ特性を踏まえた具体的な手法**を提案してください。
...
- **推奨パラメータ（GBDT）:**
  ...
  **このコンペに最適な**モデルの選定理由
  - 例：「カテゴリ変数が多いため、CatBoostを選定」
  - 例：「小さい物体の検出が必要なため、YOLOv8を選定」
- CVとLBが一致する検証環境の設計方針
- 推奨パラメータ範囲（汎化性能重視）

**⚠️ 重要:** このコンペの評価指標とデータ特性を踏まえて、最適なモデルを選定してください。
#### 【Phase 3の成果物】
- 選定したモデルとそのCVスコア
- CVとLBが一致する検証環境のコード
- OOFスコアとLBスコアの明記
...
- **メッセージ:**
  - 「なぜこの設定にしたの設計（必要なパラメータリスト）
- **このコンペで説得力のある図**のリスト（3〜5個）
  - 例：「Train vs Originalの分布比較図」
  - 例：「Feature Importanceのトップ20」
- 再現性を保証するための要件リスト
- **このコンペ向けの**公開用タイトル案（2〜3個）
  - 例：`[LB 0.xxx] XGBoost + Original Concat + Robust CV`

**⚠️ 重要:** このコンペ固有の知見を強調したタイトルとメッセージを考えてください。物】
...
- **4h-6h (Phase 3):** モデル選定と検証環境の構築
  ...
  - CVとLBが一致する検証環境を構築
  - OOF/LBスコアを確認
あなたの分析を基に、以下を明確にしてください：

1. **公開用ノートブックの設計書**
   - Clean: 必要なConfig項目リスト
   - Insightful: 載せるべき図のリスト
   - Reproducible: 再現性を保証する要件

2. **推奨タイトル案（2〜3個）**
   - 例：`[LB 0.xxx] {モデル名} + {このコンペの必殺技} Baseline`

3. **冒頭メッセージの骨子**
   - このコンペの特徴（違和感）
   - なぜこの手法が効くのか
   -必ず`docs/`フォルダ内のドキュメントを読んでから分析する**  
✅ **一般論ではなく、「このコンペでは」という具体的な分析を提供する**  
✅ **「強いモデル」ではなく「正しい戦い方」を教える**  
✅ **シンプルで再現性の高い手法を選ぶ**  
✅ **CVとLBが一致する検証環境を提供する**  
✅ **過学習を避け、汎化性能を重視する**  
✅ **見せ方にこだわり、Voteを集める**  
✅ **最後に「次にコード化すべきもの」を明確にする**

---

## 【このプロンプトの使い方】

1. **コンペ開始時：** このプロンプトをCopilotに投げる
2. **Copilotの分析を確認：** Phase 1〜4の分析結果を読む
3. **優先実装リストを基にコード化：** `train.py`, `eda.py`などを実装
4. **8時間以内に公開：** Kaggle Notebookとして公開してVoteを集める

**成功を祈っています！次のコンペでは、あなたが主役です！**作成
  - 公開（タイトル決定、メッセージ作成）
