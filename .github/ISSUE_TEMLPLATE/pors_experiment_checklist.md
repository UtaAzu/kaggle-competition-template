...
## 配置（ローカル/Repo）
- 軽量成果物 → `experiments/<EXP_ID>/artifacts/`
  - submission.csv → `experiments/<EXP_ID>/artifacts/submission.csv`
  - oof.csv → `experiments/<EXP_ID>/artifacts/oof.csv`
  - metrics.json → `experiments/<EXP_ID>/artifacts/metrics.json`
- ノートブック → `experiments/<EXP_ID>/notebooks/`
- 小図/ログ → `experiments/<EXP_ID>/artifacts/plots/`
