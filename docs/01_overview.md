...
This repository is a generic Kaggle competition template. It provides an opinionated project structure and scripts for training, inference, validation, EDA, and experiment management. Example configurations and historical competition artifacts can be placed in `examples/`.
...
This repository does not enforce a particular evaluation metric. Example metrics and notebooks are provided under `examples/` for historical or example competitions. Use `src/metrics/` to add any evaluation code you need.
