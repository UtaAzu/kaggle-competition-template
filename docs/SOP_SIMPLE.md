...
- Save:
  - Kaggle実行 → `/kaggle/working/artifacts/<EXP_ID>/` に出力
  - ローカル実行 → `artifacts/models/<EXP_ID>/`

3) 推論・提出（Where / What / Save）
- Where: Kaggle Notebook（Add Data で学習成果物を追加）
- What: 推論を実行し `/kaggle/working/submission.csv`（row_id,rule_violation）を作成
...
- 必須で repo に入れるもの（`experiments/<EXP_ID>/` 内）
  ...
  - `experiments/<EXP_ID>/artifacts/`（軽量物: plots, metrics, oof）
...
- [ ] 推論を実行し、`/kaggle/working/submission.csv` を生成して提出
...
- [ ] 軽量成果物（plots, oof, metrics.json）を `experiments/<EXP_ID>/artifacts/` に保存
