...
2. Kaggleの「Create Dataset」からそのフォルダをアップロード（名前例: `<code-dataset-slug>`）
   - 任意のノートブックで「Add Data」→ そのDatasetを追加し、先頭セルでブートストラップ
   ```
   !cp -r /kaggle/input/<code-dataset-slug>/src ./src
   !cp -r /kaggle/input/<code-dataset-slug>/configs ./configs
   !cp /kaggle/input/<code-dataset-slug>/train.py .
   !cp /kaggle/input/<code-dataset-slug>/predict.py .
   ```
4. 実行
   ```
   !pip -q install pyyaml
...
- 学習完了後に `<WORKING_DIR>/outputs/<exp_name>` を「Create Dataset」で公開
