...
def main():
    ...
    kaggle_notebook = prompt("Kaggle Notebook名 (例: mabe-linear-models)", "TBD")
    kaggle_version = prompt("Kaggle Version (例: 1)", "TBD")
    
    run = {
        "experiment_id": exp_id,
        "title": f"{exp_id} experiment",
        "system": exp_id[-1] if exp_id.startswith("EXP") and len(exp_id) >= 7 else "T",
        "kaggle_notebook": kaggle_notebook,  # ← 追加
        "kaggle_version": int(kaggle_version) if str(kaggle_version).isdigit() else None,  # ← 追加
