...
def find_train():
    candidates = [
        ...
        Path(os.environ.get('KAGGLE_DATASET_TRAIN', '/kaggle/input/<dataset-slug>/train.csv'))  # Kaggle env path if provided
    ]
    for p in candidates:
        if p.exists():
            try:
...
def discover_experiments(root=EXP_ROOT):
    ...
    # CV strategy variants (discover any notebooks/CV-strategy under experiments)
    for nb_dir in root.glob('*/notebooks/*/CV-strategy'):
        if not nb_dir.exists():
            continue
        for variant in nb_dir.iterdir():
