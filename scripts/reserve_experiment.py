# python
# filepath: /workspaces/Kaggle-Competition-Template/scripts/reserve_experiment.py
#!/usr/bin/env python3
"""
...
VALID_SYSTEMS = ["L", "D", "E", "T"]  # L=Linear, D=Deep, E=Ensemble, T=Test
...
def create_minimal_config(config_path: Path, exp_id: str, title: str = "", system: str = "") -> None:
    """
    baseline.yaml風のコメント付き・可読性重視のYAMLを生成
    """
    template = f"""\
# Experiment ID and Title
exp_id: {exp_id}
title: "{title or f"{exp_id} experiment"}"

# Paths (auto-detect Kaggle/Local)
data_root: null  # Local="./", Kaggle="/kaggle/input/MABe-mouse-behavior-detection"
...
# Feature settings
target_behaviors: "single"
time_window: 10
threshold: 0.27

# Mode
validate: false  # false=submit, true=validation
verbose: true
"""
...
def create_config_template(exp_id: str, system: str, base_template: Optional[str] = None, title: str = "") -> Path:
    ...
    if base_template:
        ...
        if base_config_path.exists():
            ...
            # train.pyのConfigクラスにないフィールドを除去
            allowed_keys = {
                "exp_id", "title", "data_root", "output_root", "n_splits", "random_state",
                "group_col", "model_type", "n_estimators", "learning_rate", "max_train_samples",
                "target_behaviors", "time_window", "threshold", "validate", "verbose"
            }
            config = {k: v for k, v in config.items() if k in allowed_keys}
            config["exp_id"] = exp_id
            config["title"] = title or f"{exp_id} experiment"
            with open(config_path, 'w', encoding="utf-8") as f:
                yaml.dump(config, f, default_flow_style=False, sort_keys=False)
            return config_path
        else:
            create_minimal_config(config_path, exp_id, title, system)
            return config_path
    else:
        create_minimal_config(config_path, exp_id, title, system)
        return config_path
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python scripts/reserve_experiment.py --system L --title "tfidf_with_char_ngrams"
  python scripts/reserve_experiment.py --system D --title "roberta_base" --template "tfidf_baseline"
"""
    )

    parser.add_argument("--system", choices=VALID_SYSTEMS, required=True, help="Experiment system: L=Linear, D=Deep, E=Ensemble, T=Test")
    ...
    try:
        ...
        config_path = create_config_template(exp_id, args.system, args.template, full_title)
